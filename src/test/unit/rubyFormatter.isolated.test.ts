...
describe('RubyFormatter (Isolated)', () => {
    ...
    describe('Rails helpers formatting', () => {
        it('should format link_to helper', () => {
            const input = 'link_to("Home",root_path)';
            const result = formatter.format(input, options);
            expect(result).to.equal('link_to("Home", root_path)');
        });

        it('should format form helpers', () => {
            const input = 'form_with(model:@user,local:true)';
            const result = formatter.format(input, options);
            expect(result).to.equal('form_with(model: @user, local: true)');
        });
    });
